...
func (h *Handler) RegisterOnMux(mux *http.ServeMux) {
	...
	mux.Handle("POST /api/models", h.authMiddleware(http.HandlerFunc(h.handleAddModel)))
	mux.Handle("PATCH /api/models/{index}", h.authMiddleware(http.HandlerFunc(h.handleUpdateModel)))
	mux.Handle("DELETE /api/models/{index}", h.authMiddleware(http.HandlerFunc(h.handleDeleteModel)))
